...
namespace GDD4000_Engines_CatMouseGame
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void LoadContent()
        {
            ...
            minDistanceToCheck = (float)Math.Pow(texture.Width / 2, 2) + (float)Math.Pow(texture.Width / 2, 2);
            minDistanceToCheck = (float)Math.Sqrt(minDistanceToCheck);
            Console.WriteLine("Min distance to check: " + minDistanceToCheck);
        }
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            //Recalculates whenever we press spacebar.
        ...
        private void DetectCollisions(int k, int count)
        {
            for (int i = k; i < k + count - 1; ++i)
            {
                for (int j = i + 1; j < sprites.Count; ++j)
                {
                    //If it crashes here, it is because you added new sprites to the count, but that sprite hasn't been initialized yet... I think...
                    if (Vector2.Distance(((Sprite)sprites[i]).Position, ((Sprite)sprites[j]).Position) < minDistanceToCheck)
                    {
                        ((AnimatedSprite)sprites[i]).Collide(((AnimatedSprite)sprites[j]));
                    }
                }
