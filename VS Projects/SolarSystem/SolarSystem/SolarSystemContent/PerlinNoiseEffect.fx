...
#define DARK_BLUE float4(0.05, 0.05, 0.45, 1.0)
#define DARKER_BLUE float4(0.03, 0.03, 0.35, 1.0)
#define WHITE float4(1, 1, 1.0, 1.0)
...
float4 marble_color(float m) {
	...
	c[9] = DARKER_BLUE;     // Middle ring, DARKER_BLUE
	c[10] = DARKER_BLUE;    // Outer ring, DARKER_BLUE
	c[11] = float4(0, .5, 0, 1); //green    // Outer fills, PALE_BLUE
	c[12] = DARKER_BLUE;
...
//My Stuff
//My Stuff
float4 PixelShaderFunction_MercuryCraters(VertexShaderOutput input) : COLOR0
{
	float LightIntensity = .9;
	float Amplify = .5;
	float NoiseScale = .2;
	float4 Color2 = float4(0.4, 0.4, 0.4, 1.0);
...
float4 PixelShaderFunction_MarsCraters(VertexShaderOutput input) : COLOR0
{
	float LightIntensity = .9;
float Amplify = .5;
float NoiseScale = .2;
float4 Color2 = float4(0.4, .2, 0, 1.0);
float4 Color1 = float4(0.9, .3, 0, 1.0);
...
float4 PixelShaderFunction_Clouds(VertexShaderOutput input) : COLOR0
{
	float LightIntensity = 1;
	float Amplify = .9;
	float NoiseScale = CloudsEffect;   //  0 to 1
	float Bias = .9;         // -1 to 1
	float4 Color1 = float4(0.5, 0.5, 0.5, 1.0);
	float4 Color2 = float4(1, 1, 1, 1.0);
	...
	output.rgb = lerp(Color1.rgb, Color2.rgb, intensity) * LightIntensity;
	if ((output.r + output.g + output.b) / 3 < .75)
	{
		discard;
	}
	return output;
...
float4 PixelShaderFunction_Sun(VertexShaderOutput input) : COLOR0
{
	float LightIntensity = .9;
	float Amplify = .5;
	float NoiseScale = SunEffect;
	float4 Color2 = float4(.8, 0.0, 0.0, 1.0); //Dark red
	float4 Color1 = float4(1.0, 1.0, 0.0, 1.0); //yellow
...
technique Sun
{
	...
	{
		VertexShader = compile vs_3_0 VertexShaderFunction();
		PixelShader = compile ps_3_0 PixelShaderFunction_Sun();
	}
}


float4 PixelShaderFunction_Mercury(VertexShaderOutput input) : COLOR0
{
	float LightIntensity = 1;
	float Amplify = .05;
	float NoiseScale = .2;   //  0 to 1
	float Bias = .9;         // -1 to 1
	float4 Color1 = float4(0.3, 0.3, 0.3, 1.0);
	float4 Color2 = float4(0.9, 0.9, 0.9, 1.0);
...
technique Mercury
{
	...
	{
		VertexShader = compile vs_3_0 VertexShaderFunction();
		PixelShader = compile ps_3_0 PixelShaderFunction_Mercury();
	}
	pass Pass2
	{
		VertexShader = compile vs_3_0 VertexShaderFunction();
		PixelShader = compile ps_3_0 PixelShaderFunction_MercuryCraters();
	}
}

float4 PixelShaderFunction_Venus(VertexShaderOutput input) : COLOR0
{
	float NoiseAmp = 1;
	float NoiseScale = .25;
	float LightIntensity = .9;
	...
	float4 BLUE = float4(0.9, 0.8, 0, 1.);
...
technique Venus
{
	...
	{
		VertexShader = compile vs_3_0 VertexShaderFunction();
		PixelShader = compile ps_3_0 PixelShaderFunction_Venus();
	}
}

float4 PixelShaderFunction_Earth(VertexShaderOutput input) : COLOR0
{
	float LightIntensity = .9;
	float Amplify = .3;
	float NoiseScale = .1;
...
technique Earth
{
	...
	{
		VertexShader = compile vs_3_0 VertexShaderFunction();
		PixelShader = compile ps_3_0 PixelShaderFunction_Earth();
	}
	pass Pass2
	{
		VertexShader = compile vs_3_0 VertexShaderFunction();
		PixelShader = compile ps_3_0 PixelShaderFunction_Clouds();
	}
}

float4 PixelShaderFunction_Mars(VertexShaderOutput input) : COLOR0
{
	float LightIntensity = 1;
	float Amplify = .05;
	float NoiseScale = .2;   //  0 to 1
	float Bias = .9;         // -1 to 1
	float4 Color1 = float4(0.6, 0.2, 0.0, 1.0);
	float4 Color2 = float4(0.9, .3, 0.0, 1.0);
...
technique Mars
{
	...
	{
		VertexShader = compile vs_3_0 VertexShaderFunction();
		PixelShader = compile ps_3_0 PixelShaderFunction_Mars();
	}
	pass Pass2
	{
		VertexShader = compile vs_3_0 VertexShaderFunction();
		PixelShader = compile ps_3_0 PixelShaderFunction_MarsCraters();
	}
}

float4 PixelShaderFunction_Jupiter(VertexShaderOutput input) : COLOR0
{
	float NoiseAmp = 1;
	float NoiseScale = .25;
	float LightIntensity = .9;
	...
	float4 Brown = float4(0.7, 0.5, 0, 1.);
	...
	float4 output = lerp(Brown, white, t);
	output.rgb *= LightIntensity;

	return output;
}

float4 PixelShaderFunction_JupiterSpot(VertexShaderOutput input) : COLOR0
{
	float4 red = float4(.6, 0, 0, 1);
	float4 startPos = float4(-35, -35, -35, 0);
	float d = 35;
	float4 output;
	if (sqrt(pow(input.wPosition.x - startPos.x, 2) + pow(input.wPosition.y - startPos.y, 2) + pow(input.wPosition.z - startPos.z, 2)) < d)
	{
		output = red;
	}
	else
...
technique Jupiter
{
	...
	{
		VertexShader = compile vs_3_0 VertexShaderFunction();
		PixelShader = compile ps_3_0 PixelShaderFunction_Jupiter();
	}
	pass Pass2
	{
		VertexShader = compile vs_3_0 VertexShaderFunction();
		PixelShader = compile ps_3_0 PixelShaderFunction_JupiterSpot();
	}
}


float4 PixelShaderFunction_Saturn(VertexShaderOutput input) : COLOR0
{
	float NoiseAmp = 1;
	float NoiseScale = 1;
	float LightIntensity = .9;
	...
	float4 Brown = float4(0.9, 0.7, .3, 1.);
	float4 white = float4(.9, .9, .7, 1);
...
technique Saturn
{
	...
	{
		VertexShader = compile vs_3_0 VertexShaderFunction();
		PixelShader = compile ps_3_0 PixelShaderFunction_Saturn();
	}
}

float4 PixelShaderFunction_Uranus(VertexShaderOutput input) : COLOR0
{
	float NoiseAmp = 1;
	float NoiseScale = 1;
	float LightIntensity = .9;
	...
	float4 Brown = float4(.53, .81, .98, 1.);
	float4 white = float4(.53, .81, .91, 1);
...
technique Uranus
{
	...
	{
		VertexShader = compile vs_3_0 VertexShaderFunction();
		PixelShader = compile ps_3_0 PixelShaderFunction_Uranus();
	}
}



float4 PixelShaderFunction_Neptune(VertexShaderOutput input) : COLOR0
{
	float NoiseAmp = 1;
	float NoiseScale = .2;
	float LightIntensity = .9;
	...
	float4 white = float4(0, 0, 1, 1.);
	float4 Brown = float4(.9, .9, 1, 1);
...
technique Neptune
{
	...
	{
		VertexShader = compile vs_3_0 VertexShaderFunction();
		PixelShader = compile ps_3_0 PixelShaderFunction_Neptune();
	}
}


float4 PixelShaderFunction_Ring(VertexShaderOutput input) : COLOR0
{
	float4 g1 = float4(.5, .5, 0, 1);
	float4 g2 = float4(.7, .7, 0, 1);
	float4 g3 = float4(.9, .9, 0, 1);
	float4 startPos = float4(0, 0, 0, 0);
	float d = 21.5;
	float4 output;
	float curD = sqrt(pow(0 - startPos.x, 2) + pow(input.wPosition.y - startPos.y, 2) + pow(input.wPosition.z - startPos.z, 2));
	if (curD > d && fmod(floor(curD * 1.9), .8) == 0)
	{
		output = g1;
		//output = float4(curD / 10, curD / 10, curD / 10, 1);
	}
	else if (curD > d && fmod(floor(curD * 1.9), 1.2) == 0)
	{
		output = g2;
	}
	else if (curD > d && fmod(floor(curD * 1.9), 1.5) == 0)
	{
		output = g3;
	}
	else
...
float4 PixelShaderFunction_URing(VertexShaderOutput input) : COLOR0
{
	float4 g1 = float4(.1, .2, .5, 1);
	float4 g2 = float4(.2, .3, .7, 1);
	float4 g3 = float4(.3, .4, .9, 1);
	float4 startPos = float4(0, 0, 0, 0);
	float d = 21.5;
	float4 output;
	float curD = sqrt(pow(0 - startPos.x, 2) + pow(input.wPosition.y - startPos.y, 2) + pow(input.wPosition.z - startPos.z, 2));
...
technique URing
{
	...
	{
		VertexShader = compile vs_3_0 VertexShaderFunction();
		PixelShader = compile ps_3_0 PixelShaderFunction_URing();
	}
}