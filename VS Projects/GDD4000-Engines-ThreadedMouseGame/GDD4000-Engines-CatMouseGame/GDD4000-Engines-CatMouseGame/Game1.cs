...
namespace GDD4000_Engines_CatMouseGame
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (sprites.Count > spritesPerThread)
            {
                leftovers = sprites.Count % spritesPerThread;
                for (int i = 0; i < sprites.Count - 1; i += spritesPerThread)
                {
                    int tracker = i;
                    int spritesPerThreadTracker = spritesPerThread;
                    task = Task.Factory.StartNew(() => DetectCollisions(tracker, spritesPerThreadTracker));
                }
            }
            else
            {
                leftovers = sprites.Count;
            }

            //Have main handle the leftovers
            DetectCollisions(sprites.Count - leftovers, leftovers);            
        }
