...
public class ToolSceneController : MonoBehaviour
{
        ...
        foreach (TileData t in grid)
        {
            switch (t.type)
            {
                case TileType.Collider:
                    ...
                    Gizmos.DrawLine(t.bl, t.br);
                    Gizmos.DrawLine(t.tl, t.tr);
                    Gizmos.DrawLine(t.bl, t.tl);
                    Gizmos.DrawLine(t.tr, t.br);
                    break;
                ...
                case TileType.PacManSpawn:
                    ...
                    Gizmos.DrawLine(t.tl, t.br);
                    break;
                case TileType.BlinkySpawn:
                    Gizmos.color = Color.red;
                    Gizmos.DrawLine(t.bl, t.tr);
                    Gizmos.DrawLine(t.tl, t.br);
                    break;
                case TileType.OtherGhostSpawn:
                    Gizmos.color = Color.magenta;
                    Gizmos.DrawLine(t.bl, t.tr);
                    Gizmos.DrawLine(t.tl, t.br);
                    break;
                case TileType.NormalDot:
                    Gizmos.color = Color.white;
                    Gizmos.DrawLine(t.bl + new Vector2(.4f, .4f), t.tr - new Vector2(.4f, .4f));
                    Gizmos.DrawLine(t.tl + new Vector2(.4f, -.4f), t.br - new Vector2(.4f, -.4f));
                    break;
    ...
    {
        currentTool = TileType.Teleporter;
    }

    /// <summary>
    /// Called when the button to get the pac man spawn tool is clicked
    /// </summary>
    public void ButtonClickPacManSpawn()
    {
        currentTool = TileType.PacManSpawn;
    }
    ...
    /// <summary>
    /// Called when the button to get the blinky spawn tool is clicked
    /// </summary>
    public void ButtonClickBlinkySpawn()
    {
        currentTool = TileType.BlinkySpawn;
    }

    /// <summary>
    /// Called when the button to get the other ghost spawn tool is clicked
    /// </summary>
    public void ButtonClickOtherGhostSpawn()
    {
        currentTool = TileType.OtherGhostSpawn;
    }

    /// <summary>
    /// Called when the button to get the normal dot tool is clicked
    /// </summary>
    public void ButtonCLickNormalDot()
    {
        currentTool = TileType.NormalDot;
    }

    /// <summary>
    /// Called when the button to get the power dot tool is clicked
    /// </summary>
    public void ButtonClickPowerDot()
    {
        currentTool = TileType.PowerDot;
    }
    ...
    /// <summary>
    /// This method will handle these tile types:
    /// Teleporter, PacManSpawn, BlinkySpawn, OtherGhostSpawn, NormalDot, and PowerDot
    /// </summary>
    private void HandleAllSingleNodeTiles()
    {
        foreach (TileData t in grid)
        {
            switch(t.type)
            {
                case TileType.Teleporter:
                    GameObject o = new GameObject("Teleporter");
                    o.transform.SetParent(spriteRenderer.transform);
                    o.transform.position = t.GetTileWorldPosition();
                    o.AddComponent<BoxCollider2D>();
                    o.GetComponent<BoxCollider2D>().offset = new Vector2(.5f, .5f);
                    o.AddComponent<TeleporterIdentifier>();
                    o.tag = "Teleporter";
                    break;
                case TileType.PacManSpawn:
                    GameObject p = new GameObject("PacManSpawn");
                    p.transform.SetParent(spriteRenderer.transform);
                    p.transform.position = t.GetTileWorldPosition();
                    p.AddComponent<BoxCollider2D>();
                    p.GetComponent<BoxCollider2D>().offset = new Vector2(.5f, .5f);
                    p.tag = "PacManSpawn";
                    break;
                case TileType.BlinkySpawn:
                    GameObject b = new GameObject("BlinkySpawn");
                    b.transform.SetParent(spriteRenderer.transform);
                    b.transform.position = t.GetTileWorldPosition();
                    b.AddComponent<BoxCollider2D>();
                    b.GetComponent<BoxCollider2D>().offset = new Vector2(.5f, .5f);
                    b.tag = "BlinkySpawn";
                    break;
                case TileType.OtherGhostSpawn:
                    GameObject g = new GameObject("OtherGhostSpawn");
                    g.transform.SetParent(spriteRenderer.transform);
                    g.transform.position = t.GetTileWorldPosition();
                    g.AddComponent<BoxCollider2D>();
                    g.GetComponent<BoxCollider2D>().offset = new Vector2(.5f, .5f);
                    g.tag = "OtherGhostSpawn";
                    break;
                case TileType.NormalDot:
                    GameObject d = new GameObject("NormalDot");
                    d.transform.SetParent(spriteRenderer.transform);
                    d.transform.position = t.GetTileWorldPosition();
                    d.AddComponent<BoxCollider2D>();
                    d.GetComponent<BoxCollider2D>().offset = new Vector2(.5f, .5f);
                    d.tag = "NormalDot";
                    break;
                case TileType.PowerDot:
                    GameObject a = new GameObject("PowerDot");
                    a.transform.SetParent(spriteRenderer.transform);
                    a.transform.position = t.GetTileWorldPosition();
                    a.AddComponent<BoxCollider2D>();
                    a.GetComponent<BoxCollider2D>().offset = new Vector2(.5f, .5f);
                    a.tag = "PowerDot";
                    break;
            }
        }
    ...
    private void GenerateInteractableObjects()
    {
        ...
        foreach (TileData t in grid)
        {
            ...
            if (!collidersAlreadyTakenCareOf.Contains(t) && t.type == TileType.Teleporter)
            {
...
public enum TileType
{
    ...
    Teleporter,   //A trigger collider that will do something when the player steps on this object.
    PacManSpawn,
    BlinkySpawn,
    OtherGhostSpawn,
    NormalDot,
