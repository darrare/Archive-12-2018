...
public class Acknowledgement : LgJsonDictionary, IJsonable
{
	...
	public void HandleNewObject(int connectionId)
	{
		if (ack == 1) {
			...
			Application.LoadLevel ("ScoreTracking");
		} else {
			Debug.Log ("Connection Failure...");
			GameObject.Find ("WarningText").GetComponent<Text>().text = "Incorrect event password";
		}
	}
}
...
public class StaffInstruction : LgJsonDictionary, IJsonable
{
	...
	public void HandleNewObject(int connectionId)
	{
		GameObject.Find ("HomeTeam").GetComponent<Text> ().text = homeTeam;
		GameObject.Find ("AwayTeam").GetComponent<Text> ().text = awayTeam;
		GameObject.Find ("FieldNumber").GetComponent<Text> ().text = "Field number: " + fieldNum.ToString ();
	}
}
